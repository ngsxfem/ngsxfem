...
namespace ngfem
{
  ...
  template <int D>
  class TraceLaplaceIntegrator : public BilinearFormIntegrator
  {
    shared_ptr<CoefficientFunction> coef;
  public:
    TraceLaplaceIntegrator (const Array<shared_ptr<CoefficientFunction>> & coeffs)
      : coef(coeffs[0]){;}
    virtual ~TraceLaplaceIntegrator(){ ; };
  ...
  template <int D>
  class TraceLaplaceBeltramiIntegrator : public BilinearFormIntegrator
  {
    shared_ptr<CoefficientFunction> coef;
  public:
    TraceLaplaceBeltramiIntegrator (const Array<shared_ptr<CoefficientFunction>> & coeffs)
      : coef(coeffs[0]){;}
    virtual ~TraceLaplaceBeltramiIntegrator(){ ; };
  ...
  template <int D>
  class TraceConvectionIntegrator : public BilinearFormIntegrator
  {
    shared_ptr<CoefficientFunction> conv;
  public:
    TraceConvectionIntegrator (const Array<shared_ptr<CoefficientFunction>> & coeffs)
      : conv(coeffs[0]){;}
    virtual ~TraceConvectionIntegrator(){ ; };
