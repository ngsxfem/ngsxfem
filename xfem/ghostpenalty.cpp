...
namespace ngfem
{
  ...
  template <int D>
  void LowOrderGhostPenaltyIntegrator<D>::CalcFacetMatrix (const FiniteElement & volumefel1, int LocalFacetNr1,
    ...
    for (dt=POS; dt<IF; dt=(DOMAIN_TYPE)((int)dt+1))
    {
      ...
      for (int k = 0; k < ir_time.GetNIP(); k++)
      {
        for (int l = 0; l < ir_facet.GetNIP(); l++)
        {
          ...
          elmat += delta * lam * orthdist * len1 * tau * ir_time[k].Weight() * ir_facet[l].Weight() * bmat * Trans (bmat);
        }
      }
    }
  }
  ...
  static RegisterBilinearFormIntegrator<LowOrderGhostPenaltyIntegrator<2> > init_gp_2d ("lo_ghostpenalty", 2, 3);
  static RegisterBilinearFormIntegrator<LowOrderGhostPenaltyIntegrator<3> > init_gp_3d ("lo_ghostpenalty", 3, 3);
  static RegisterBilinearFormIntegrator<LowOrderGhostPenaltyIntegrator<2> > init_gp_2d_st ("stx_lo_ghostpenalty", 2, 5);
  static RegisterBilinearFormIntegrator<LowOrderGhostPenaltyIntegrator<3> > init_gp_3d_st ("stx_lo_ghostpenalty", 3, 5);
