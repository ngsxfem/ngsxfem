...
class LevelSetMeshAdaptation:
    """
Class to compute a proper mesh deformation to improve
a piecewise (multi-) linear level set approximation
to obtain a higher order accurate approximation.

Computes a continuous function that describes 
a shift between points that are on the (P1 ) 
approximated level set function and its higher
order accurate approximation. 
The transformation is only applied on elements 
where a level value inside a certain interval
(lower,upper) exists.

The result is a deform function (D) which is computed 
pointwise as
...
This class holds its own members for the 
higher order an lower order (P1) approximation
of the level set function and only depends on 
the input of a mesh and a CoefficientFunction
...
The computed deformation depends on different options:
  ...
  order : int
    order of deformation GridFunction
    (ideally) order + 1 is the accuracy of the
    geometry approximation after applying the deformation
    on the mesh 
  ...
  lset_upper_bound: float
    largest relevant level set value to define the 'cut'
    elements where the mapping should be applied
    
  threshold: float
    maximum (pointwise) value for d(x)/h in the mapping
      Psi(x) = x + d(x) qn(x)
    of the mesh transformation. A small value might 
    be necessary if the geometry is only coarsely 
    approximated to avoid irregular meshes after a 
    corresponding mesh deformation.

  discontinuous_qn: boolean
    As an approximation for the normal direction 
    we use 
    n_h = nabla phi_h (gradient of higher order level set 
    approximation)
    depending on the discontinuous_qn flag this 
    normal field will be projected onto a continuous
    finite element space or not.
...
See documentation of xfem.CalcMaxDistance 
    ...
    def MarkForRefinement(self, levelset = None, refine_threshold = 0.1, absolute = False):
        """
Marks elements for refinement where the geometry
approximation is larger than a prescrbed 
relative (or absolute) (to the mesh size) error.
This will lead to a refinement in zones with high
curvature.

Parameters:

  levelset : ngsolve.CoefficientFunction/None
    accurate level set function that is used 
    to compute the error. If None self.lset_ho
    will be used.

  refine_threshold : float
    value that decides if a geometry error is 
    large enough to mark for refinement or not

  absolute : boolean
    decides if the refine_threshold is an 
    absolute value or if it is weighted with 
    the mesh size
        """
