...
numproc projectshift nppsh -levelset=lset_ho -levelset_p1=lset_p1 -deform=deform -quasinormal=qn -lset_lower_bound=$(lset_lower_bound) -lset_upper_bound=$(lset_upper_bound) -threshold=1.5
...
# (1+pi/2-sqrt(2)*cos(pi/4*(x*x*x*x))),

# cos(pi/4*(r44))
# -sin(...)*pi * xi3
# -cos(...)*pi*pi*r66-sin(...)*pi*3*r22

# -sqrt(2)*pi*aneg*(cos(...)*pi*r66+sin(...)*3*r22)
...
numproc vtkoutput npout -filename=d104_almost_cube_2d_
