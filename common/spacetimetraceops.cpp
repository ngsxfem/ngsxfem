...
protected:
  ...
  GridFunction & u_tr;
public:
  TraceEvaluator(const GridFunction & a_u_st, GridFunction & a_u_trace)
  ...
  void EvaluateTrace(double time, LocalHeap & lh)
  {
    ...
    const SpaceTimeFESpace & fes_st = dynamic_cast<const SpaceTimeFESpace &> (u_st.GetFESpace());
    const FESpace & fes_tr = u_tr.GetFESpace();
