...
print("ndof",Vhc.ndof)
gf = []
gf.append(GridFunction(Compress(H1(mesh,order=1),active_dofs=GetActiveDof(mesh))))
for i in range(nref):
    mesh.Refine()
    Vh.Update()
    lsetp1.Update()
    ...
    nmesh = meshonlvl(i)
    gf.append(GridFunction(Compress(H1(nmesh,order=1),active_dofs=GetActiveDof(nmesh))))
