...
def get_l2error(order, n_ref, deform):
    ...
    l2error = sqrt(NewIntegrateX(lset=lset_approx,mesh=mesh,cf=err_sqr_coefs[0],order=2*order,domain_type=NEG,heapsize=1000000, use_saye=False) + NewIntegrateX(lset=lset_approx,mesh=mesh,cf=err_sqr_coefs[1],order=2*order,domain_type=POS,heapsize=1000000, use_saye=False))
