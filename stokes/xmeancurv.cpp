...
namespace ngfem
{
  ...
  template<int D>
  void XLBMeanCurvIntegrator<D> ::
  CalcElementVector (const FiniteElement & base_fel,
                     const ElementTransformation & eltrans, 
                     FlatVector<double> elvec,
                     LocalHeap & lh) const
  {
    static Timer timer ("XLBMeanCurvIntegrator::CalcElementMatrix");
  ...
  template class XLBMeanCurvIntegrator<2>;
  template class XLBMeanCurvIntegrator<3>;

  static RegisterLinearFormIntegrator<XLBMeanCurvIntegrator<2> > initstxh1cut2d ("xLBmeancurv", 2, 1);
  static RegisterLinearFormIntegrator<XLBMeanCurvIntegrator<3> > initstxh1cut3d ("xLBmeancurv", 3, 1);
