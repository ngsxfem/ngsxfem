...
from kite3D_in_sympy import get_Laplace_u_str, get_dt_rho
...
cube = CSGeometry()
cube.Add (OrthoBrick(Pnt(-3.5,-1.5,-1.5), Pnt(3.5,1.5,1.5)))
ngmesh = cube.GenerateMesh(maxh=2*(1/2)**i, quad_dominated=False)
mesh = Mesh (ngmesh)
...
rho =  (1 - 2.5*y**2 - 2.5*z**2)*t*sin(pi*t)
rhoL = lambda t: (1- 2.5*y**2 - 2.5*z**2)*t*sin(pi*t)
rho_str = "(1 - 2.5*y**2 - 2.5*z**2)*t*sin(pi*t)"

#convection velocity:
d_rho = eval(get_dt_rho(rho_str))

w = CoefficientFunction((d_rho,0,0))

# level set
r = sqrt((x- rho)**2+y**2+z**2)
levelset= r - r0
levelsetL = lambda t: sqrt((x- rhoL(t))**2+y**2+z**2) - r0

Draw(levelsetL(0.), mesh,"lset", autoscale=False, min = 0, max = 0)

Q = pi/r0
u_exact = cos(Q*r) * sin(pi*t)
u_exactL = lambda t: cos(Q*sqrt((x- rhoL(t))**2+y**2+z**2)) * sin(pi*t)
