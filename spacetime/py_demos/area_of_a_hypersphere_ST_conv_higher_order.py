...
def area_of_a_hypersphere_ST_error(n_steps = 64, i=3, structured_mesh= True):
    ...
    for i in range(n_steps):
        ...
        print("Calc deformation")
        deformation = lset_adap_st.CalcDeformation(levelset,tref,told, delta_t)
        
        print("Set deformation")
        mesh.SetDeformation(deformation)
        
        print("Integration")
        val_vol = Integrate({ "levelset" : lset_p1, "domain_type" : NEG}, CoefficientFunction(1.0), mesh, order= space_order+2, time_order = time_order+2)
        val_int = Integrate({ "levelset" : lset_p1, "domain_type" : IF}, CoefficientFunction(1.0), mesh, order= space_order+2, time_order = time_order+2)
