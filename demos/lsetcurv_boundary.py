...
The standard mesh adaptation assumes the zero level set stays inside the
domain; if it *crosses* the boundary, the deformation pushes the boundary
mesh off the domain and the cut geometry loses its higher order.  Passing
``boundary_tangential=True`` (optionally a boundary name / regex / list /
Region to restrict it) keeps the deformation tangential to the boundary
(u.n = 0) and recovers the O(h^(k+1)) accuracy.  The mesh needs a sound
boundary parametrisation for ``specialcf.normal`` (e.g. a netgen mesh).

Example: a circle crossing the bottom edge of [-1,1]^2 at 45 degrees -- we
report the cut-area error and the (machine-zero) total-area defect that
certifies the boundary is preserved.
"""
...
for n in (8, 16, 32, 64):
    ...
    lsetdom = {"levelset": adap.lset_p1, "domain_type": NEG,
               "order": 2 * order + 2}
    area = Integrate(levelset_domain=lsetdom, cf=one, mesh=mesh,
                     deformation=deform)
    err = abs(area - area_exact)

    # u.n = 0 keeps the deformed domain the rectangle, so its area stays 4
    mesh.SetDeformation(deform)
    tot_defect = abs(NgsIntegrate(one, mesh, order=2 * order) - 4.0)
