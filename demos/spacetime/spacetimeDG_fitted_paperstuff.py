...
outfile = open("tikz_mesh_fitted.tex", "w")
name = "trig"

for t in mesh.ngmesh.Elements2D():
    p1 = mesh.ngmesh.Points()[t.points[0]]
    p2 = mesh.ngmesh.Points()[t.points[1]]
    p3 = mesh.ngmesh.Points()[t.points[2]]
    
    print(mesh.deformation(p1))
                
    outfile.write("\\draw["+name+"] ( "+str(p1[0])+" , "+str(p1[1])+" ) -- ( "+str(p2[0])+" , "+str(p2[1])+" ) -- ( "+str(p3[0])+" , "+str(p3[1])+" ) -- cycle;\n")
outfile.close()
