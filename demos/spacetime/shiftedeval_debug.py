...
fes = H1(mesh, order=order, dirichlet=[1, 2, 3, 4])
...
exact = sin(10 * y)
gfu_old.Set(exact)
l2error_old = sqrt(Integrate((gfu_old - exact) * (gfu_old - exact), mesh,
                             order=2 * order + 1))
...
print("L2-error(new):", sqrt(Integrate((gfu_new - exact) * (gfu_new - exact),
                                       mesh, order=2 * order + 1)))
