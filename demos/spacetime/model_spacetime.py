...
fes1 = V = H1(mesh, order=order, dirichlet=[1, 2, 3, 4])
...
def u_exact(t):
    return CoefficientFunction(sin(pi * t) * sin(pi * x)**2 * sin(pi * y)**2)


coeff_f = CoefficientFunction(
    pi * cos(pi * t) * sin(pi * x)**2 * sin(pi * y)**2
    - 2 * pi * pi * sin(pi * t) * (cos(pi * x)**2 * sin(pi * y)**2
                                   - sin(pi * x)**2 * sin(pi * y)**2
                                   + cos(pi * y)**2 * sin(pi * x)**2
                                   - sin(pi * x)**2 * sin(pi * y)**2))
...
print("Assemble A")
a = BilinearForm(st_fes, symmetric=False)
a += SymbolicBFI(levelset_domain=lset_neg, form=delta_t * grad(u) * grad(v),
                 time_order=2)
...
while tend - t_old > delta_t / 2:
    ...
    # clear storage
    f = LinearForm(st_fes)
    f += SymbolicLFI(levelset_domain=lset_neg, form=delta_t * coeff_f * v,
                     time_order=2)
