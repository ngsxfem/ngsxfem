...
a += lsetadap.BFI(NEG, alpha[0] * gradu[0] * gradv[0])
a += lsetadap.BFI(POS, alpha[1] * gradu[1] * gradv[1])
# Nitsche integrals:
a += lsetadap.BFI(IF, (average_flux_u * (v[0] - v[1]) +
                       average_flux_v * (u[0] - u[1]) +
                       stab * (u[0] - u[1]) * (v[0] - v[1])))

f = LinearForm(VhG)
# r.h.s. domain integrals:
f += lsetadap.LFI(NEG, coef_f[0] * v[0])
f += lsetadap.LFI(POS, coef_f[1] * v[1])
...
DrawDC(lsetadap.lset_p1, gfu.components[1], gfu.components[0],
       mesh, "gfu", deformation=lsetadap.deform)

# Error coefficients:
err_sqr_coefs = [(gfu.components[i] - solution[i])**2 for i in [0, 1]]

# Computation of L2 error:
l2error = sqrt(lsetadap.Integrate(NEG, cf=err_sqr_coefs[0], order=2*order)
               + lsetadap.Integrate(POS, cf=err_sqr_coefs[1], order=2*order))
