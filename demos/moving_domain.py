...
basis_in_time = {"modal": ([lambda t: 1.0, lambda t: t,
                            lambda t: (2 * t - 1) * (2 * t - 1)],
                           [lambda t: 0.0, lambda t: 1,
                            lambda t: 4 * (2 * t - 1)])}
...
levelset = CoefficientFunction(sqrt(x * x + (y - rho)**2) - r0)

# right hand side
coeff_f = CoefficientFunction(-(pi/r0) * r1
                              * (sin(r1 * sqrt(x**2 + (y - rho)**2))
                                 * sin(r1 * sqrt(x**2 + (y - rho)**2))
                                 - cos(r1 * sqrt(x**2 + (y - rho)**2))
                                 * cos(r1 * sqrt(x**2 + (y - rho)**2)))
                              + (pi / r0) * cos(r1 * sqrt(x**2 + (y - rho)**2))
                              * sin(r1 * sqrt(x**2 + (y - rho)**2))
                              * (1 / sqrt(x**2 + (y - rho)**2)))

# exact solution for monitoring the error
exact = CoefficientFunction(cos(r1 * sqrt(x**2 + (y - rho)**2))
                            * cos(r1 * sqrt(x**2 + (y - rho)**2)))
...
u0 = GridFunction(V)
# added DG Jumps for Ghost-penalty terms
W = FESpace([V for _l in range(k_t + 1)], flags={"dgjumps": True})
gfu = GridFunction(W)
Draw(u0, mesh, "unew")

# stabilization parameter for Ghost-penalty
gamma_stab = [10**-_l for _l in range(1, k_s + 1)]


class SlabDiscretization:
    '''
    Class for discretisation on the time slab.
    '''
    ...
    def Update(self, delta_t):
        ...
        # add dummy time point (with weight 0) if initial time is not
        # part of time quad:
        if 0.0 not in qr.shifted_pts(0, 1):
        ...
        if 1.0 not in qr.shifted_pts(0, 1):
        ...
        # mesh adaptation
        self.lsetmeshadap = LevelSetMeshAdaptation(
            mesh, order=k_s, threshold=0.1, discontinuous_qn=True)
        self.levelset_domain = {
            "levelset": self.lsetmeshadap.lset_p1, "domain_type": NEG}
        ...
        # jump of the normal derivative (needed for Ghost-Penalty
        # stabilization)
        u_dnjump_ti = [sum([phi[n](tref) * dnjump(u[n], _l + 1, n)
                            for n in range(k_t + 1)]) for _l in range(k_s)]
        v_dnjump_ti = [sum([phi[n](tref) * dnjump(v[n], _l + 1, n)
                            for n in range(k_t + 1)]) for _l in range(k_s)]

        # loop over discrete time points to set up the integrators
        # the actual linear system is assembled in the SolveProblem
        # function below.
        for t_i, omega_i in zip(self.points_time_ref, self.weights_time_ref):
            ...
            form_inner_rhs = SymbolicLFI(self.levelset_domain,
                                         form=inner_term_rhs)
            ...
            if omega_i != 0.0:
                # Ghost-Penalty Terms
                for _l in range(k_s):
                    gp_term = delta_t * omega_i * \
                        gamma_stab[_l] * power(h, 2 * _l + 1) * \
                        u_dnjump_ti[_l] * v_dnjump_ti[_l]
                    form_new_gp = SymbolicBFI(gp_term, skeleton=True)
                    ai += form_new_gp
                    marked_integrators_at_ti["gpfacets_spacetime"].append(
                        form_new_gp)
...
def SolveProblem(sd, delta_t):
    ...
    while tend - tnew > delta_t / 2:
        ...
        # elements that have been in haspos and in hasneg should be
        # marked as hasif!
        ...
        ba_facets = GetFacetsWithNeighborTypes(mesh, a=hasneg_spacetime,
                                               b=hasif_spacetime,
                                               bnd_val_a=False,
                                               bnd_val_b=False)
        ...
        for i, t_i in enumerate(sd.points_time_ref):
            ...
            # Collect the integrators from slab discretisation and inform them
            # about the current cut-situation.
            for key in markers:
                if (key in sd.marked_integrators[i]
                        and len(sd.marked_integrators[i][key]) > 0):
        ...
        u0.vec[:] = 0.0
        for _l in range(k_t + 1):
            u0.vec.data += phi[_l](1.0) * gfu.components[_l].vec
